...
def Worker(A, B, n, a, c, Br, Bc):
	...
	for k in range(0, r): #desde 0 hasta N filas restantes
		C.append(A[a+k-r]) #añade N filas a la lista
	print("THR EXTRA:",a*(n+k)-r)
	tid = thr.Thread(target=Final, args=(C, B, a*(n+k)-r, Br, Bc))
	tid.start()
	tid.join()
...
for i in range(USR): #desde 0 hasta: N threads definidos por el user
	...
	for j in range (0, rpt): #desde 0 hasta: K filas asignadas por thread
		C.append(A[rpt*i+j])  #añade a la lista las proximas K*N filas
...
>>>>>>> 5cd22a136b7c211b316ba16b1e1a08cb74824ddd
